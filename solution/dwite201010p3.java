...
public final class dwite201010p3 extends DwiteSolution {
	...
	private static int getTileCount(int width, int height) {
		if (width == 0 || height == 0)
			...
			return getTileCount(width / 2, height / 2);
		else if (width % 2 == 0 && height % 2 == 1)
			return width + getTileCount(width / 2, height / 2);
		else if (width % 2 == 1 && height % 2 == 0)
			return height + getTileCount(width / 2, height / 2);
		else  // width % 2 == 1 && height % 2 == 1
			return width + height - 1 + getTileCount(width / 2, height / 2);
	}
